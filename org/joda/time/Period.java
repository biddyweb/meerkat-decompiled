  ...
  implements ReadablePeriod, Serializable
{
  ...
  public Period normalizedStandard(PeriodType paramPeriodType)
  {
    ...
    Period localPeriod = new Period(getMillis() + 1000L * getSeconds() + 60000L * getMinutes() + 3600000L * getHours() + 86400000L * getDays() + 604800000L * getWeeks(), localPeriodType, ISOChronology.getInstanceUTC());
    int i = getYears();
    int j = getMonths();
    if ((i != 0) || (j != 0))
    {
      ...
      {
        int m = FieldUtils.safeToInt(l / 12L);
        localPeriod = localPeriod.withYears(m);
        l -= m * 12;
      }
      if (localPeriodType.isSupported(DurationFieldType.MONTHS_TYPE))
      {
        int k = FieldUtils.safeToInt(l);
  ...
  public Days toStandardDays()
  {
    ...
    return Days.days(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd((getMillis() + 1000L * getSeconds() + 60000L * getMinutes() + 3600000L * getHours()) / 86400000L, getDays()), 7L * getWeeks())));
  }

  public Duration toStandardDuration()
  {
    checkYearsAndMonths("Duration");
    return new Duration(getMillis() + 1000L * getSeconds() + 60000L * getMinutes() + 3600000L * getHours() + 86400000L * getDays() + 604800000L * getWeeks());
  }

  public Hours toStandardHours()
  {
    checkYearsAndMonths("Hours");
    return Hours.hours(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd((getMillis() + 1000L * getSeconds() + 60000L * getMinutes()) / 3600000L, getHours()), 24L * getDays()), 168L * getWeeks())));
  }

  public Minutes toStandardMinutes()
  {
    checkYearsAndMonths("Minutes");
    return Minutes.minutes(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd((getMillis() + 1000L * getSeconds()) / 60000L, getMinutes()), 60L * getHours()), 1440L * getDays()), 10080L * getWeeks())));
  }

  public Seconds toStandardSeconds()
  {
    checkYearsAndMonths("Seconds");
    return Seconds.seconds(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(getMillis() / 1000, getSeconds()), 60L * getMinutes()), 3600L * getHours()), 86400L * getDays()), 604800L * getWeeks())));
  }

  public Weeks toStandardWeeks()
  {
    checkYearsAndMonths("Weeks");
    long l = getMillis() + 1000L * getSeconds() + 60000L * getMinutes() + 3600000L * getHours() + 86400000L * getDays();
    return Weeks.weeks(FieldUtils.safeToInt(getWeeks() + l / 604800000L));
  }
  ...
  {
    PeriodType localPeriodType = DateTimeUtils.getPeriodType(paramPeriodType);
    if (localPeriodType.equals(getPeriodType()))
      return this;
    return new Period(this, localPeriodType);
  }
