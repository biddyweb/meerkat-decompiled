...
class ReportUploader
{
  ...
  boolean forceUpload(Report paramReport)
  {
    synchronized (this.fileAccessLock)
    {
      try
      {
        Context localContext = Crashlytics.getInstance().getContext();
        CreateReportRequest localCreateReportRequest = new CreateReportRequest(new ApiKey().getValue(localContext), paramReport);
