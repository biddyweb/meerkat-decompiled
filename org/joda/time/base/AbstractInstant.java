  ...
  implements ReadableInstant
{
  ...
  {
    if (this == paramObject);
    ReadableInstant localReadableInstant;
    do
    {
      return true;
      if (!(paramObject instanceof ReadableInstant))
        return false;
      localReadableInstant = (ReadableInstant)paramObject;
    }
    while ((getMillis() == localReadableInstant.getMillis()) && (FieldUtils.equals(getChronology(), localReadableInstant.getChronology())));
    return false;
  }
  ...
  {
    Chronology localChronology = DateTimeUtils.getChronology(getChronology()).withZone(paramDateTimeZone);
